...
-- Iterator to iterate over the paths of trees generated by `g` and
-- extending `path0`.
newTreeIterIO :: Path a -> GeneratorIO a -> IO (TreeIterIO a)
newTreeIterIO path0 g = newIterIO next (Just $ pathToStack path0)
  where
...
f3 g =
  ((paths . generate) g ==) <$> (runIterIO <=< newTreeIterIO []) (return . g)
...
-- Monadic iterator to iterate over the paths of trees generated by `g`,
...
newPruneTreeIterIO :: (Path a -> IO Bool) -> Path a -> GeneratorIO a
                   -> IO (TreeIterIO a)
newPruneTreeIterIO p path0 g = newTreeIterIO path0 g' >>= skipUntilIO p
  where
