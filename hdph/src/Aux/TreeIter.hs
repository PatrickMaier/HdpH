...
-- test: `generate` and `generateM` agree.
tst3 = (generate g ==) <$> generateM (return . g)
...
newPruneTreeIterM :: (MonadIO m)
                  => (Path a -> Bool) -> Path a -> GeneratorM m a
                  -> m (TreeIterM m a)
{-# INLINABLE newPruneTreeIterM #-}
newPruneTreeIterM p path0 g = newTreeIterM path0 g' >>= skipUntilM (return . p)
  where
    g' path | p path    = return []  -- p True  --> prune path
            | otherwise = g path     -- p False --> generate path extension
...
f4 n d g = (lhs g ==) <$> rhs (return . g)
  where
    lhs = filter p . paths . generate
    rhs = runIterM <=< bufferM n <=< newPruneTreeIterM p []
    p = (== d) . length
