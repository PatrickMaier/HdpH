...
module Aux.TreeIter
  ...
  , newPruneTreeIter
    ...
    -- * tree iterators in the IO monad
  , TreeIterIO
  , newTreeIterIO
  , newPruneTreeIterIO
  ) where
...
import Aux.Iter
       (Iter, newIter, runIter, buffer, skipUntil,
        IterIO, newIterIO, runIterIO, bufferIO, skipUntilIO)
...
generate g = expand []
  where
    expand path = map (\x -> Tree x (expand (path ++ [x]))) $ g path

generateIO :: GeneratorIO a -> IO [Tree a]
generateIO g = expand []
  where
    expand path = mapM (\x -> Tree x <$> expand (path ++ [x])) =<< g path
...
-- test: `degenerate` fails as right inverse when there duplicate paths.
...
newTreeIter :: Generator a -> TreeIter a
newTreeIter g = newIter next (Just [])
  where
...
-- minimally satisfying `p` (ie. paths with no proper prefix satisfying `p`).
newPruneTreeIter :: (Path a -> Bool) -> Generator a -> TreeIter a
newPruneTreeIter p g = newTreeIter g' `skipUntil` p
  where
...
-- * `runIter . (`buffer` n) . newPruneTreeIter p` where `p = (== d) . length`
...
-------------------------------------------------------------------------------
-- Iterator in IO over paths in a tree (depth-first, left-to-right)

-- Monadic iterator over paths (of trees).
type TreeIterIO a = IterIO (Maybe (Stack a)) (Path a)

-- Iterator to iterate over the paths of trees generated by `g`.
newTreeIterIO :: GeneratorIO a -> IO (TreeIterIO a)
newTreeIterIO g = newIterIO next (Just [])
  where
 -- next :: Maybe (Stack a) -> IO (Maybe (Maybe (Stack a), Path a))
...
{-
-- TESTING
-- test function: compare `paths . generate` to `runIterIO <=< newTreeIterIO`
f3 g = ((paths . generate) g ==) <$> (runIterIO <=< newTreeIterIO) (return . g)
...
-------------------------------------------------------------------------------
-- Pruning iterator in IO over paths in a tree (depth-first, left-to-right)

-- Monadic iterator to iterate over the paths of trees generated by `g` and
-- minimally satisfying `p` (ie. paths with no proper prefix satisfying `p`).
-- Note that the action `p` maybe called many times; its side effects
-- should therefore be idempotent.
newPruneTreeIterIO :: (Path a -> IO Bool) -> GeneratorIO a -> IO (TreeIterIO a)
newPruneTreeIterIO p g = newTreeIterIO g' >>= skipUntilIO p
  where
...
-- test function: compare
-- * `filter p . paths . generate` and
-- * `runIterIO <=< bufferIO n <=< newPruneTreeIterIO (return . p)`
-- where `p = (== d) . length`
f4 n d g = (lhs g ==) <$> rhs (return . g)
  where
    lhs = filter p . paths . generate
    rhs = runIterIO <=< bufferIO n <=< newPruneTreeIterIO (return . p)
    p = (== d) . length
