...
main = do
  ...
  -- HdpH magic (register static decls and parse cmd line options)
    ...
    then do
      ...
      ((bigCstar, info), t_compute) <- timeM' $ case version of
        ...
        1 -> -- sequential maxclique search based on tree iterator
             force <$> maxClqIter bigG debug
        2 -> -- sequential maxclique search based on depth-bounded tree iterator
             -- for generating "tasks" (that are executed sequentially)
             force <$> maxClqBndIter bigG depth debug
        3 -> -- parallel maxclique search with task generated by depth-bounded 
             -- tree iterator
             force <$> maxClqBndIterPar bigG depth workers debug
