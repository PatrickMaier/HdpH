...
GenerateNodes := function(parent, cutoff, delta, triv_stop)
  ...
  ## return no children if target arc size is reached or candidates exhausted
  if Size(parent.arc) >= parent.k or IsEmpty(parent.cands) then
    return [];
  fi;
  ...
  if Size(parent.arc) >= 4 then
    if Size(parent.cands) < cutoff + Size(parent.arc) * delta then
      return [];
    fi;
  fi;
...
#  Sort(prechildren, ByDescOrbAscStab);  # (b)
  ...
  for prechild in prechildren do
    ...
    if Size(parent.arc) >= 4 then
      ...
      if Size(candidates) < cutoff + Size(parent.arc) * delta then
        ## adjust candidate set of current node
        parent.cands := candidates;

        ## add current node as a child (to make it a frontier node)
        ## if it has a chance of reaching a k-arc, else prune
        if Size(parent.arc) + Size(parent.cands) >= parent.k then
          Add(children, parent);
        fi;

        ## stop generation and return children
        return children;
      fi;
    fi;
    ...
    ## add child only if arc is small or has a chance of reaching a k-arc
    if Size(child.arc) <= 4 or Size(child.arc) + Size(child.cands) >= child.k
    then
      Add(children, child);
    fi;
...
#nodes := EnumerateSearchTree(PG(2,23), 9, 100, 50, false);;
#PrintSearchTreeNodes(nodes, "PG_2_23_cutoff100_delta50.txt");

#nodes := EnumerateSearchTree(PG(2,27), 9, 100, 50, true);;
#PrintSearchTreeNodes(nodes, "PG_2_27_cutoff100_delta50_stop.txt");

#nodes := EnumerateSearchTree(PG(2,29), 10, 200, 50, false);;
#PrintSearchTreeNodes(nodes, "PG_2_29_cutoff200_delta50.txt");
