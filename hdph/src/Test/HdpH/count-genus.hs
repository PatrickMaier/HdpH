...
genus :: SGP -> Int
genus (SGP _ _ _ _ g) = g

conductor :: SGP -> Int
conductor (SGP _ _ c _ _) = c
...
-- Enumeration of numerical semigroups as an infinite lazy list;
-- genus increases monotonically, i.e. 'sgps' corresponds to a breadth-first
-- traversal of the semigroups tree generated by the algorithm in [1].
sgps :: [SGP]
sgps = bfs children [root]

-- Returns the index into list 'sgps' of the first semigroup with genus 'g'.
indexOfFirstSGPWithGenus :: Int -> Integer
indexOfFirstSGPWithGenus g =
  fst $ head $ dropWhile ((< g) . genus . snd) $ zip [0 ..] sgps

-- Breadth first walk through tree generated by 'g', working off queue 'q'.
bfs :: (a -> [a]) -> [a] -> [a]
bfs g (x:q) = x : bfs g (q ++ g x)
bfs _ []    = []
...
lookupSeqA007323 g
  ...
  | otherwise                       = Nothing
    where
      -- sequence as stored in OEIS [http://oeis.org/]
      seqA007323 :: [Integer]
...
-- * index of semigroup (in list 'sgps') to use as root
parseArgs :: [String] -> (Int, Int, Int, Int)
parseArgs []     = (def_ver, def_g, def_seq_levels, def_ridx)
parseArgs (s:ss) =
  let go :: Int -> [String] -> (Int, Int, Int, Int)
      go v []           = (v, def_g,   def_seq_levels, def_ridx)
      go v [s1]         = (v, read s1, def_seq_levels, def_ridx)
      go v [s1,s2]      = (v, read s1, read s2,        def_ridx)
...
def_ver, def_g, def_seq_levels, def_ridx :: Int
def_ver        =  1  -- version
def_g          = 30  -- genus
def_seq_levels = 12  -- bottom levels of tree to traverse sequentially
...
main = do
  ...
  let (version, g, seq_levels, ridx) = parseArgs args
  initrand seed
  case version of
