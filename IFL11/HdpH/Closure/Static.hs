...
--
-- Differences between 'HdpH.Closure.Static' and a builtin 'Static' are:
-- * Static values must be declared at toplevel
-- * Static terms must be explicitly named (via 'staticAs')
-- * Static terms must be explicitly declared and registered before use


module HdpH.Closure.Static
...
import HdpH.Closure.Static.State (sTblRef)
import HdpH.Closure.Static.Type
...
--
-- * Only the serialisable reference is actually serialised when a Static term 
...
--
-- * There are two ways of obtaining a term 'stat :: Static t'. By introducing
...
-- >   ...
--
-- The module 'HdpH.Closure' offers more convenient support for declaring
-- and registering Static terms linked to explicit closures.
...
instance Serialize (Static a) where
  ...
  get = resolve <$> Data.Serialize.get
        -- NOTES:
        -- * Hyperstrictness is enforced by 'resolve' (and by construction
        --   of the Static table).
        -- * Serialisation crashes if the Static term is not yet registered.
        -- * Deserialisation crashes if the index passed to 'resolve' is
...
--
-- * Static terms can only be introduced by the smart constructor 'staticAs'.
--
-- * We assume that the 'label' part of a Static term (as constructed by 
...
--
-- * Likewise, we assume that the 'index' part of a Static term is a /key/.
...
getIndex :: AnyStatic -> StaticIndex
getIndex (Any stat) = index stat
...
-- Registers the given Static declaration, turning it into the global
-- Static table. Must be called exactly once, before any operations
-- on Static terms.
register :: StaticDecl -> IO ()
register sd = do
  ...
  unless ok $
    error "HdpH.Closure.Static.register: 2nd attempt"
...
-- Convert the contents of the global Static table to a list of Strings,
-- one per entry; to be called only after registration.
showStaticTable :: [String]
