...
import Control.Parallel.HdpH.Internal.Type.Location
       (Node, MyNodeException(MyNodeUnset))
...
myNodeRef :: IORef Node
myNodeRef = unsafePerformIO $ newIORef $ throw MyNodeUnset
{-# NOINLINE myNodeRef #-}    -- required to protect unsafePerformIO hack


-----------------------------------------------------------------------------
-- reference to list of all node IDs;
-- will be set (to non-empty list) in module HdpH.Internal.Comm

allNodesRef :: IORef [Node]
